...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && isValidHttpUrl(source) && category && textLength <= 250) {
      ...
      // 4. Add the new facy to the UI: add the fact to state
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      ...
      <input
        value={source}
        type="text"
        placeholder="Enter Trustworthy Source.."
...
function Fact({ fact, setFacts }) {
  ...
  async function handleVote(coulmnName) {
    setIsUpdating(true);
    const { data: updatedFact, error } = await supabase
      .from("facts")
      .update({ [coulmnName]: fact[coulmnName] + 1 })
      .eq("id", fact.id)
      .select();
    setIsUpdating(false);
